...
use Kladislav\JWTAuth\Exceptions\PayloadException;
...
class Payload implements ArrayAccess, Arrayable, Countable, Jsonable, JsonSerializable
{
     ...
     *
     * @var \Kladislav\JWTAuth\Claims\Collection
     */
    private $claims;
     ...
     * Build the Payload.
     *
     * @param  \Kladislav\JWTAuth\Claims\Collection  $claims
     * @param  \Kladislav\JWTAuth\Validators\PayloadValidator  $validator
     ...
     *
     * @return \Kladislav\JWTAuth\Claims\Collection
     */
     ...
     * @param  mixed  $value
     *
     * @throws \Kladislav\JWTAuth\Exceptions\PayloadException
     */
    ...
    public function __call($method, $parameters)
    {
        if (preg_match('/get(.+)\b/i', $method, $matches)) {
            foreach ($this->claims as $claim) {
                if (get_class($claim) === 'Kladislav\\JWTAuth\\Claims\\'.$matches[1]) {
